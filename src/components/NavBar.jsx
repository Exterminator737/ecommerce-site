...
const NavBar = () => {
  ...
  const { search, setSearch, categories, setShowSearch, getCartCount } =
  ...
  return (
    <div>
      <div className="flex items-center justify-between py-3 lg:py-5 font-medium lg:px-4 sm:px-[2vw] md:px-[4vw] md:mx-[-3vw] lg:mx-[-4vw] ">
        <div className="flex gap-10 items-center">
          <Link className="flex items-center gap-1 lg:gap-5" to="/">
            <img src={assets.logo} className="w-7 lg:w-20" alt="logo" />
          ...
          <div
            ...
            className="hidden border rounded-xl h-fit  hover:rounded-b-none px-2 relative gap-2 tracking-wider lg:flex justify-center items-center py-1 text-gray-900 hover:text-[--Lime]  cursor-pointer"
          >
            ...
            {
              <div
                className={`bg-white text-gray-900 absolute top-[100%] h-[70vh] left-0 w-full flex flex-col gap-2 py-2 overflow-hidden rounded-b-2xl ${
                  catVisible ? "" : "hidden"
                }`}
              >
                {" "}
        ...
        <div className="flex items-center gap-6">
          ...
          <img
            onClick={() => {setVisible(true); setHideCat(true)}}
      ...
      {/* --------------- CATEGORIES DROPDOWN FOR SMALL AND MEDIUM DEVICES ---------------- */}
      <div
        onClick={() => setCatVisible(!catVisible)}
        onMouseOver={() => setCatVisible(true)}
        onMouseOut={() => setCatVisible(false)}
      className={`${hideCat ? 'hidden' : ''} flex w-fit text-sm rounded-xl h-fit  hover:rounded-b-none relative gap-2 tracking-wider lg:hidden justify-center items-center text-gray-900 hover:text-[--Lime]  cursor-pointer`}
      >
        <img className="w-6" src={assets.ham} alt="menu icon" />
        {<p>CATEGORIES</p>}
        {
          <div
            className={`bg-white text-gray-900 absolute top-[100%] h-[100vh] left-0 w-fit flex flex-col gap-2 py-2 rounded-b-2xl ${
              catVisible ? "" : "hidden"
            }`}
          >
            {" "}
      ...
      <div
        className={`${
          show ? "" : "hidden"
        } inline-flex items-center justify-center border border-gray-400 pl-3 pr-2 py-1 rounded-full mt-5 w-full sm:w-1/20 bg-[--Main]`}
      >
        <input
          value={search}
